...
function WebCam({setFingerCount}: { setFingerCount: (fingerCount: number) => void }) {
    ...
    const updateStableFinger = (value: number) => {
        ...
        const now = Date.now();
        if (now - lastChangeTime >= 1000) { // stable for 1s
            if (stableValue !== lastValue) {
                stableValue = lastValue;
                setFingerCount(stableValue); // only update after 1s
            }
        }
    };
    ...
    const countFingers = (lms: { x: number; y: number; z: number }[]) => {
        ...
        // Thumb – horizontal check (right hand)
    ...
    return (
        <Webcam
            ...
            width={400}
            height={356}
            className="w-[400px] h-[356px] rounded-xl shadow-lg border border-gray-300 object-cover"
        />
